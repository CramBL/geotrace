...
- **Breaking:** C++ `FileBuilder::add_nav_fix`, `add_satellite_report`, `add_annotation`, `add_event_marker`, `add_event` and `add_channel` throw `std::out_of_range` for a `Timestamp` whose count is past the range a UTC timestamp covers, `INT64_MIN` included, where the SDK recorded a nav fix or a satellite report without an `INT64_MIN` timestamp beside a valid one. Built without exceptions, the builder records `GTD_ERR_OUT_OF_RANGE` with a message stating the argument and the count.
