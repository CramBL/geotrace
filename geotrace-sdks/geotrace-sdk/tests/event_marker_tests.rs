...
fn valid_simple() {
    ...
    sink.add_nav_fix(fix(0, 55.0, 12.0));
    sink.add_event_marker(marker("power/turn_on", 0));
}
...
fn valid_kebab() {
    let mut sink = NavFileBuilder::new().open();
    sink.add_nav_fix(fix(0, 55.0, 12.0));
    sink.add_event_marker(marker("agps/request-epo/gps", 0));
}
...
fn valid_single_segment() {
    let mut sink = NavFileBuilder::new().open();
    sink.add_nav_fix(fix(0, 55.0, 12.0));
    sink.add_event_marker(marker("boot", 0));
}
...
fn valid_mixed_case_and_digits() {
    let mut sink = NavFileBuilder::new().open();
    sink.add_nav_fix(fix(0, 55.0, 12.0));
    sink.add_event_marker(marker("sensor/GPS3/lock", 0));
}
...
fn markers_are_stored_in_nav_file() {
    let mut sink = NavFileBuilder::new().open();
    sink.add_nav_fix(fix(0, 10.0, 20.0));
    sink.add_nav_fix(fix(100, 12.0, 22.0));
    sink.add_event_marker(marker("power/on", 0));
    sink.add_event_marker(marker("power/off", 100));
...
fn position_interpolated_at_midpoint() {
    let mut sink = NavFileBuilder::new().open();
    sink.add_nav_fix(fix(0, 10.0, 20.0));
    sink.add_nav_fix(fix(100, 12.0, 24.0));
    sink.add_event_marker(marker("sensor/mid", 50));
...
fn position_clamped_to_first_fix_when_before_track() {
    ...
    sink.add_nav_fix(fix(10, 55.0, 12.0));
    sink.add_event_marker(marker("boot", 0));
...
fn add_event_auto_registers_icon_for_derived_enum() {
    let mut sink = NavFileBuilder::new().open();
    sink.add_nav_fix(fix(0, 55.0, 12.0));
    sink.add_event(&IconOuter::Power(IconLeaf::TurnOn), t(0));
    sink.add_event(&IconOuter::Power(IconLeaf::Failed), t(1));
