...
- **Interface:** Fixed the force quit confirmation vanishing when the last background write finishes: it now stays up saying the work finished, with the force quit grayed out and a Close button, and closes itself four seconds later, or when the pointer leaves it, whichever is later.
