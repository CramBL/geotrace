...
- **Interface:** Redesigned query results into a tabbed panel below the editor: a line per query stating what it matched, a sortable table of every match of the run, and the picked match's points under shared column headers, every number over a bar for its size against the whole run's matches, with TSV export.
