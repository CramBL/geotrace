...
use gt_side_panel::test_util::{self, FIXES_PER_TRACK, PanelState};
...
fn middle_track() -> DateTime<Utc> {
    test_util::utc(0, 0, 0) + Duration::days(3)
}
...
fn a_drag_to_the_left_end_of_the_active_range_bar_keeps_the_earlier_track_filtered_out() {
    ...
    let from = test_util::point_on_track(active_bar, 0.02);
    harness
        .inner
        .press_drag_release(from, test_util::point_on_track(active_bar, -0.02) - from, 1);
    harness.run();
...
fn the_active_range_bar_states_the_ends_of_the_window_and_not_of_its_viewport() {
    ...
    let mut harness = test_util::harness(vec![file]);
...
fn drag_along_the_bar(
    ...
    let from = test_util::point_on_track(bar, drag.press);
    harness
        .inner
        .press_drag_release(from, test_util::point_on_track(bar, drag.release) - from, 1);
    harness.run();
}
...
fn a_drag_past_the_other_handle_on_the_only_bar_stops_a_second_short_of_it(
    ...
    let mut harness = test_util::harness(vec![test_util::recording(
        "one_stint.gtd",
...
fn every_drag_keeps_at_least_the_fine_minimum_between_the_window_bounds() {
    ...
    for bar_index in [0, 1] {
        for (from, to) in drags {
            let Some(bar) = test_util::bar_rects(&harness).get(bar_index).copied() else {
                continue;
            };
            let from = test_util::point_on_track(bar, from);
            harness
                .inner
...
fn the_label_under_the_coarse_bar_shows_the_bound_after_a_drag_clamps_it() {
    ...
    let labels = test_util::label_texts(&harness, "01/03");
    assert_eq!(
...
fn the_coarse_bar_names_the_active_range_bar_on_hover() {
    ...
    harness
        .inner
        .hover_at_and_settle(test_util::point_on_track(bar, 0.5), 3);

    assert_eq!(
        test_util::label_texts(&harness, "active range bar below")
            .first()
            .map(String::as_str),
...
fn the_only_bar_states_no_minimum_on_hover() {
    ...
    assert!(test_util::label_texts(&harness, "active range bar below").is_empty());
}
