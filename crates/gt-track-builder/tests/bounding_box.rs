...
use gt_track_builder::test_util;
...
fn bounding_box_across_the_antimeridian_covers_the_span_the_track_flew() {
    let bounds = test_util::measured_geometry(&antimeridian_track())
        .expect("every fix has a recorded position")
        .bounding_box;

    test_util::assert_degrees_close(bounds.lon.start().as_degrees(), 179.0);
    test_util::assert_degrees_close(bounds.lon.span_degrees(), 1.5);
}
...
fn bounding_box_center_across_the_antimeridian_lands_on_the_track() {
    ...
    let geometry =
        test_util::measured_geometry(&points).expect("every fix has a recorded position");
...
fn merc_bounds_across_the_antimeridian_wrap_at_the_world_edge() {
    let merc_bounds = test_util::measured_geometry(&antimeridian_track())
        .expect("every fix has a recorded position")
        .merc_bounds;
    ...
    assert!(
        (width - 1.5 / 360.0).abs() < test_util::DEGREES_TOLERANCE,
...
fn bounding_box_of_a_local_track_is_tight_and_holds_every_fix() {
    ...
    test_util::assert_degrees_close(bounds.lon.start().as_degrees(), 12.0);
    test_util::assert_degrees_close(bounds.lon.end().as_degrees(), 12.5);
    test_util::assert_degrees_close(bounds.lat.south().as_degrees(), 54.9);
    test_util::assert_degrees_close(bounds.lat.north().as_degrees(), 55.2);
...
fn bounding_box_of_fixes_at_one_position_is_degenerate(#[case] fix_count: i64) {
    ...
    let bounds = test_util::measured_geometry(&points)
        .expect("every fix has a recorded position")
        .bounding_box;

    test_util::assert_degrees_close(bounds.lon.start().as_degrees(), 151.2);
    test_util::assert_degrees_close(bounds.lon.span_degrees(), 0.0);
    test_util::assert_degrees_close(bounds.lat.south().as_degrees(), -33.9);
    test_util::assert_degrees_close(bounds.lat.north().as_degrees(), -33.9);
}
...
fn merc_bounds_put_the_northern_edge_at_y_min() {
    ...
    let merc_bounds = test_util::measured_geometry(&points)
        .expect("every fix has a recorded position")
        .merc_bounds;
...
fn bounding_box_around_the_pole_holds_every_meridian_and_reaches_the_pole() {
    let bounds = test_util::measured_geometry(&circumpolar_track())
        .expect("every fix has a recorded position")
        .bounding_box;
    ...
    test_util::assert_degrees_close(bounds.lat.south().as_degrees(), 89.9);
    test_util::assert_degrees_close(bounds.lat.north().as_degrees(), 90.0);
}
...
fn merc_bounds_around_the_pole_span_the_world_and_lie_on_its_northern_edge() {
    let merc_bounds = test_util::measured_geometry(&circumpolar_track())
        .expect("every fix has a recorded position")
        .merc_bounds;
    ...
    assert!(
        merc_bounds.x_min.abs() < test_util::DEGREES_TOLERANCE,
        "x_min"
    );
    assert!(
        (merc_bounds.x_max - 1.0).abs() < test_util::DEGREES_TOLERANCE,
        "x_max"
    );
    assert!(
        (merc_bounds.y_min - northern_edge.y).abs() < test_util::DEGREES_TOLERANCE,
        "y_min {}",
        merc_bounds.y_min
    );
    assert!(
        (merc_bounds.y_max - northern_edge.y).abs() < test_util::DEGREES_TOLERANCE,
        "y_max {}",
        merc_bounds.y_max
    );
}
...
proptest::proptest! {
    ...
    fn bounding_box_holds_every_fix_of_a_local_track(
        ...
        let points: Vec<NavPoint> = (0..n)
            ...
            .collect();
        let points = vec1::Vec1::try_from_vec(points).expect("at least one fix");
        let geometry = test_util::measured_geometry(&points).expect("every fix has a recorded position");
        for resolved in &geometry.resolved_positions {
            let (latitude, longitude) = resolved.coordinates();
