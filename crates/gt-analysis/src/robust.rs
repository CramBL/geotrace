...
mod tests {
    ...
    fn median_i64_is_none_when_empty_and_the_overflow_safe_midpoint_otherwise() {
        assert_eq!(super::median_i64(&[3, 1, 2]), Some(2));
        assert_eq!(super::median_i64(&[1, 2, 3, 4]), Some(2)); // (2 + 3) / 2, truncated
        assert_eq!(super::median_i64(&[]), None);
        assert_eq!(super::median_i64(&[7]), Some(7));
        // Midpoint of the full i64 span: computed in i128, so no overflow.
        assert_eq!(super::median_i64(&[i64::MIN, i64::MAX]), Some(0));
    }
}
