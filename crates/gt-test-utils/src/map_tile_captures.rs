//! The captured base tiles the map snapshots draw, and the
...
pub fn map_tile_capture_dir() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../tests/captures/map_tiles")
}
...
/// `just map-tile-captures` records.
#[expect(
    clippy::expect_used,
    reason = "a recording run that cannot write its list has nothing to offer"
)]
pub fn assert_map_tile_capture_is_complete(
    snapshot_name: &str,
    ...
    assert!(
        ...
        "{snapshot_name} drew {} base tiles the capture under {:?} does not hold: {}. \
         ...
         `just map-tile-captures <path>`.",
        missing.len(),
        map_tile_capture_dir(),
        missing.join(" ")
    );
}
