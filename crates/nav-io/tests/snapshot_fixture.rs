...
/// the structure produced by `nav_io::load_file`.
///
...
use naview_sdk::{
    ...
    NavFileBuilder, NavFix, Satellite as SdkSat, SatelliteReport, Utc, Velocity, degree,
    meter_per_second,
};
...
fn fixture_path() -> PathBuf {
    ...
    workspace.join("tests/fixtures/snapshot.nvd")
}
...
fn v(mps: f64) -> Velocity {
    Velocity::new::<meter_per_second>(mps)
}
...
fn build_snapshot_bytes() -> Vec<u8> {
    ...
    for i in 0..12_usize {
        ...
        b.add_satellite_report(satellite_report(pt_time, &tracked_prns, fix_prn_sets[i]));
    }
    ...
    let nav_file = b.finish().expect("all builder constraints satisfied");
