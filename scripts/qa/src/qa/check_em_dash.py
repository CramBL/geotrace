"""Forbid em-dash / box-drawing section header comments in Rust, Python, and Just files.
...
from qa._check import Violation, hash_comment_files, rs_files, run_check
...
def _collect(root: Path) -> list[Violation]:
    ...
    for path in chain(rs_files(root), hash_comment_files(root)):
