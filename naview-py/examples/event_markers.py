...
hierarchy — e.g. ``"connectivity/agps/request"`` — so NaView can
group and filter them by prefix in the Events panel.

Event markers are added via ``builder.add_event_marker()``.
...
for path, secs, note in EVENTS:
    builder.add_event_marker(EventMarker(path, START + timedelta(seconds=secs),
                                         annotation=note))

builder.add_event_marker_style(
    EventMarkerStyle("power/boot",  icon_name="lightning", color_hex="#44BB44"))
builder.add_event_marker_style(
    EventMarkerStyle("power/sleep", icon_name="pin",       color_hex="#4488FF"))

nav_file = builder.finish()

out = Path(tempfile.gettempdir()) / "naview_event_markers.nvd"
