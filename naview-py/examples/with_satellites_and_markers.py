...
builder = NavFileBuilder().set_meta(Meta(
    title="A3 Southbound — field test",
    device="u-blox ZED-F9P",
    notes="Engineer annotations recorded via tablet app",
))
...
print(f"  {len(nav_file.points)} nav points, "
      f"{len(nav_file.markers)} markers")
print()
print("Markers:")
for m in nav_file.markers:
    print(f"  {m.label!r:35s}  "
          f"@ {m.lat:.5f}, {m.lon:.5f}  "
          f"[{m.icon}]")
