...
int main(void) {
    ...
    s = gtd_builder_add_nav_fix(b, t0, gtd_ts_none(), 51.5074, -0.1278, GTD_SOME_F64(90.0),
                                GTD_SOME_F64(5.5), GTD_SOME_F64(3.2));
    if (s != GTD_OK) {
        fprintf(stderr, "add_nav_fix: %s\n", gtd_last_error());
        goto fail;
    }

    s = gtd_builder_add_satellite_report(b, t0, gtd_ts_none(), sats, sizeof sats / sizeof sats[0]);
    if (s != GTD_OK) {
        fprintf(stderr, "add_satellite_report: %s\n", gtd_last_error());
        goto fail;
    }

    s = gtd_builder_add_nav_fix(b, gtd_ts_from_seconds((uint64_t)time(NULL) + 10), gtd_ts_none(),
                                51.5080, -0.1265, GTD_SOME_F64(85.0), GTD_SOME_F64(5.8),
                                GTD_SOME_F64(2.9));
    if (s != GTD_OK) {
        fprintf(stderr, "add_nav_fix: %s\n", gtd_last_error());
        goto fail;
