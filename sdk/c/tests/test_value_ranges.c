...
Test(value_ranges, out_of_range_coordinates_read_verbatim) {
    GtdNavFile *f = NULL;
    cr_assert_eq(gtd_nav_file_open(GTD_OUT_OF_RANGE_FIXTURE_PATH, &f), GTD_OK);
    cr_assert_not_null(f);
    cr_assert_eq(gtd_nav_file_nav_point_count(f), 4);

    GtdNavPointInfo nan_lat;
    cr_assert_eq(gtd_nav_file_get_nav_point(f, 0, &nan_lat), GTD_OK);
    cr_assert(isnan(nan_lat.lat_deg));

    GtdNavPointInfo lat_91;
    cr_assert_eq(gtd_nav_file_get_nav_point(f, 1, &lat_91), GTD_OK);
    assert_near(lat_91.lat_deg, 91.0, 1e-9);

    GtdNavPointInfo lon_minus_181;
    cr_assert_eq(gtd_nav_file_get_nav_point(f, 2, &lon_minus_181), GTD_OK);
    assert_near(lon_minus_181.lon_deg, -181.0, 1e-9);

    GtdNavPointInfo heading_675;
    cr_assert_eq(gtd_nav_file_get_nav_point(f, 3, &heading_675), GTD_OK);
