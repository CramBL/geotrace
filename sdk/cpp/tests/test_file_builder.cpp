...
TEST_CASE("FileBuilder: fluent chain works end-to-end") {
    auto file = FileBuilder{}
        .device("chain test")
        .add_nav_fix(NavFix{.gps_time = t0, .lat = Angle::degrees(1.0), .lon = Angle::degrees(2.0)})
        .add_nav_fix(NavFix{.gps_time = t1, .lat = Angle::degrees(1.1), .lon = Angle::degrees(2.1)})
        .finish();
...
TEST_CASE("FileBuilder: NoNavFixesError thrown when no fixes added") {
    CHECK_THROWS_AS(FileBuilder{}.finish(), NoNavFixesError);
}
