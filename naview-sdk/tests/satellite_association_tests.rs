...
fn first_constellation(p: &naview_sdk::NavPoint) -> Constellation {
    p.satellites
        .as_ref()
...
fn window_boundary_inclusive() -> Result<(), BuildError> {
    ...
    b.add_nav_fix(fix_at(0, 55.0, 12.0));
    b.add_satellite_report(report_gps(100)); // exactly 100 ms away
...
fn window_boundary_one_microsecond_past_is_excluded() -> Result<(), BuildError> {
    let mut b = NavFileBuilder::new().with_satellite_window(Duration::milliseconds(100));
    b.add_nav_fix(fix_at(0, 0.0, 0.0));
...
fn report_goes_to_nearer_of_two_candidate_fixes() -> Result<(), BuildError> {
    ...
    b.add_nav_fix(fix_at(0, 10.0, 10.0)); // fix A — GPS constellation expected
    b.add_nav_fix(fix_at(2000, 20.0, 20.0)); // fix B — Glonass constellation expected
    // Report at t=1 800 ms: 1 800 ms from A (outside 500 ms window), 200 ms from B (inside).
    b.add_satellite_report(report_with(1800, Constellation::Gps, 1));
...
fn report_equidistant_goes_to_earlier_fix() -> Result<(), BuildError> {
    ...
    let mut b = NavFileBuilder::new().with_satellite_window(Duration::milliseconds(2000));
    b.add_nav_fix(fix_at(0, 10.0, 10.0)); // fix A
    b.add_nav_fix(fix_at(2000, 20.0, 20.0)); // fix B
    // Report at t=1 000 ms: exactly 1 000 ms from both fixes.
    b.add_satellite_report(report_gps(1000));
...
fn four_reports_matched_to_four_fixes_no_ghosts() -> Result<(), BuildError> {
    let mut b = NavFileBuilder::new();
    for i in 0..4_i64 {
        b.add_nav_fix(fix_at(i * 2000, 10.0 + i as f64, 10.0));
        // Each report is 100 ms after its corresponding fix — well within the window.
        b.add_satellite_report(report_with(
            i * 2000 + 100,
            [
                Constellation::Gps,
...
fn zero_reports_all_fixes_have_no_satellite_data() -> Result<(), BuildError> {
    let mut b = NavFileBuilder::new();
    for i in 0..3_i64 {
        b.add_nav_fix(fix_at(i * 1000, 55.0, 12.0));
    }
...
fn three_reports_one_fix_only_closest_wins() -> Result<(), BuildError> {
    ...
    let mut b = NavFileBuilder::new();
    b.add_nav_fix(fix_at(0, 55.0, 12.0));
    b.add_nav_fix(fix_at(5000, 55.1, 12.1));

    b.add_satellite_report(report_with(100, Constellation::Gps, 1)); // R1 — winner
    b.add_satellite_report(report_with(200, Constellation::Glonass, 2)); // R2 — loser
    b.add_satellite_report(report_with(300, Constellation::Galileo, 3)); // R3 — loser
...
fn equidistant_reports_earlier_one_wins() -> Result<(), BuildError> {
    ...
    let mut b = NavFileBuilder::new();
    b.add_nav_fix(fix_at(500, 55.0, 12.0));
    // Add the LATER report first to confirm it doesn't win just by insertion order.
    b.add_satellite_report(report_with(750, Constellation::Glonass, 99)); // later
    b.add_satellite_report(report_with(250, Constellation::Gps, 1)); // earlier
...
fn multiple_ghosts_after_last_fix_all_have_heading_none() -> Result<(), BuildError> {
    ...
    b.add_satellite_report(report_with(2000, Constellation::Gps, 1));
    b.add_satellite_report(report_with(3000, Constellation::Glonass, 2));
    b.add_satellite_report(report_with(4000, Constellation::Galileo, 3));
...
fn second_ghost_after_last_fix_is_further_than_first() -> Result<(), BuildError> {
    ...
    b.add_nav_fix(
        NavFix::builder()
            ...
            .heading(Angle::new::<degree>(0.0)) // heading north
            .build(),
    );
    b.add_satellite_report(report_gps(2000));
    b.add_satellite_report(report_gps(3000));

    let nav_file = b.finish()?;
    let points = nav_file.nav_points();

    assert_eq!(points.len(), 3);
    let fix_lat = points[0].fix.lat.get::<degree>();
    let g1_lat = points[1].fix.lat.get::<degree>();
    let g2_lat = points[2].fix.lat.get::<degree>();
...
fn between_fix_ghost_interpolated_at_correct_fraction() -> Result<(), BuildError> {
    ...
    let ghost_lon = ghost.fix.lon.get::<degree>();
...
fn between_fix_ghosts_evenly_distributed_when_no_delta_available() -> Result<(), BuildError> {
    ...
    let lon1 = points[1].fix.lon.get::<degree>();
    let lon2 = points[2].fix.lon.get::<degree>();
...
fn pre_fix_dropped_post_fix_ghosted_in_same_batch() -> Result<(), BuildError> {
    let mut b = NavFileBuilder::new();
    b.add_nav_fix(fix_at(5000, 0.0, 0.0));

    // Pre-first-fix report — must be dropped.
    b.add_satellite_report(report_with(0, Constellation::Gps, 1));
    // Post-last-fix report — must become a ghost.
    b.add_satellite_report(report_with(10_000, Constellation::Glonass, 2));
    ...
    for i in 0..3_i64 {
        b.add_nav_fix(
            NavFix::builder()
                ...
                .lat(Angle::new::<degree>(55.0 + i as f64 * 0.1))
                .lon(Angle::new::<degree>(12.0))
                .heading(Angle::new::<degree>(0.0))
                .build(),
        );
    ...
    for i in 0..6_i64 {
        b.add_nav_fix(
            NavFix::builder()
                .gps_time(t(i * 1_000))
                .sys_time(t(i * 1_000 + GPS_SYS_OFFSET_MS))
                .lat(Angle::new::<degree>(55.0 + i as f64 * 0.01))
                .lon(Angle::new::<degree>(12.0))
                .heading(Angle::new::<degree>(0.0))
                .build(),
        );
...
fn gps_ahead_600ms_sat_at_exactly_500ms_delay_boundary() -> Result<(), BuildError> {
    ...
    let mut b = NavFileBuilder::new().with_satellite_window(Duration::milliseconds(500));
    b.add_nav_fix(
        NavFix::builder()
            .gps_time(t(0))
            .sys_time(t(-GPS_SYS_OFFSET_MS))
...
fn no_nav_fixes_no_annotations_returns_empty() -> Result<(), BuildError> {
    let mut b = NavFileBuilder::new();
    b.add_satellite_report(report_gps(0));
    b.add_satellite_report(report_gps(1000));
