...
impl GeomagneticIndexScheduler {
    ...
    fn start_next(&mut self) {
        ...
        day_fetch_dispatch::spawn_fetch_thread(
            &self.ctx,
            &self.tx,
            "solar",
            fetch.day,
...
mod tests {
    ...
    fn kp_day(status: KpStatus) -> KpSeries {
        KpSeries {
            samples: vec![KpSample {
                status,
                ..kp_sample(fixtures::utc_instant(2026, 7, 20, 0, 0), 2.667)
            }],
        }
    }
    ...
    fn the_window_before_a_tec_peak_is_read_from_the_archived_periods(
        ...
        let archived_day = fixtures::date(2026, 7, 20);
        if archived {
        ...
        let activity =
            scheduler.activity_before_peak(one_track(), fixtures::utc_instant(2026, 7, 21, 0, 0));
    ...
    fn a_day_archived_again_is_read_into_the_window_again() {
        let (_dir, store, mut scheduler) = scheduler_with_archive();
        let archived_day = fixtures::date(2026, 7, 20);
        let peak = fixtures::utc_instant(2026, 7, 21, 0, 0);
    ...
    fn a_day_index_read_that_failed_on_another_process_is_run_again_and_finds_the_days() {
        let (_dir, store, mut scheduler) = scheduler_with_archive();
        archive_definitive_day(&store, fixtures::date(2026, 7, 20));
    ...
    fn a_day_before_every_index_begins_is_never_queued() {
        ...
        scheduler.request_days_for(TimeRange::new(
            fixtures::utc_instant(1900, 1, 1, 0, 0),
            fixtures::utc_instant(1900, 1, 1, 1, 0),
        ));
        assert_eq!(scheduler.days.queued(), 0);
        assert!(!scheduler.days.is_fetching());
    }
    ...
    fn a_day_missing_one_index_is_requested_again() {
        let (_dir, store, mut scheduler) = scheduler_with_archive();
        archive_day(&store, fixtures::date(2026, 7, 20), |archive| {
            archive.insert_or_replace_kp_day(
    ...
    fn an_overlong_recording_queues_nothing() {
        let (_dir, _store, mut scheduler) = scheduler_with_archive();
        scheduler.request_days_for(TimeRange::new(
            fixtures::utc_instant(2026, 6, 1, 0, 0),
    ...
    fn a_backfill_queues_only_the_days_the_archive_lacks() {
        let (_dir, store, mut scheduler) = scheduler_with_archive();
        for archived in [fixtures::date(2026, 7, 21), fixtures::date(2026, 7, 22)] {
            archive_definitive_day(&store, archived);
        }

        let queued = scheduler.backfill(fixtures::date(2026, 7, 20), fixtures::date(2026, 7, 26));
    ...
    fn a_fully_archived_range_queues_nothing() {
        let (_dir, store, mut scheduler) = scheduler_with_archive();
        for offset in 20..=26 {
            archive_definitive_day(&store, fixtures::date(2026, 7, offset));
        }

        assert_eq!(
            scheduler.backfill(fixtures::date(2026, 7, 20), fixtures::date(2026, 7, 26)),
            Some(0)
        );
    ...
    fn a_backfill_before_coverage_queues_nothing() {
        let (_dir, _store, mut scheduler) = scheduler_with_archive();
        assert_eq!(
            scheduler.backfill(fixtures::date(1900, 1, 1), fixtures::date(1931, 12, 31)),
            Some(0)
        );
        assert_eq!(scheduler.days.backfill_progress(), None);
    }
    ...
    /// its day.
    #[rstest]
    #[case::stored(IndexDayMessage::Stored {
        day: fixtures::date(2026, 7, 20),
        kp_samples: 8,
        hp30_samples: 48,
    })]
    #[case::failed(UnarchivedDay::failed(fixtures::date(2026, 7, 20), "boom".to_owned()).into())]
    #[case::rejected(
        UnarchivedDay::rejected(fixtures::date(2026, 7, 20), WriteRejection::ShuttingDown).into()
    )]
    fn progress_advances_on_every_outcome(#[case] message: IndexDayMessage) {
        let mut scheduler = scheduler_without_archive();
        scheduler
            .days
            .queue_backfill_of(&[fixtures::date(2026, 7, 20), fixtures::date(2026, 7, 21)]);
        assert_eq!(
    ...
    fn cancelling_keeps_the_in_flight_day() {
        let mut scheduler = scheduler_without_archive();
        let (in_flight, queued) = (fixtures::date(2026, 7, 20), fixtures::date(2026, 7, 21));
    ...
    fn cancelling_leaves_track_requested_days_alone() {
        let mut scheduler = scheduler_without_archive();
        let track_day = fixtures::date(2026, 7, 19);
        scheduler.days.queue_track_day(track_day);
        scheduler
            .days
            .queue_backfill_of(&[fixtures::date(2026, 7, 20)]);
    ...
    fn the_status_reports_the_queue_and_the_archived_recording_days() {
        ...
        scheduler.request_days_for(TimeRange::new(
            fixtures::utc_instant(2026, 7, 20, 8, 0),
            fixtures::utc_instant(2026, 7, 21, 17, 0),
        ));

        assert_eq!(
            scheduler.days.fetch_status(),
            DayFetchStatus {
                fetching: Some(fixtures::date(2026, 7, 21)),
                queued: 0,
    ...
    fn archiving_a_day_covers_the_recording_day_it_belongs_to() {
        ...
        scheduler
            .days
            .await_recording_day(fixtures::date(2026, 7, 20));
    ...
    fn an_ingested_day_archives_every_index_and_the_host() {
        ...
        let ingested = fixtures::date(2026, 7, 20);
    ...
    fn a_day_before_hp30_begins_archives_kp_alone() {
        let (_dir, store) = archive();
        let ingested = fixtures::date(1970, 1, 1);
        let transport =
    ...
    fn a_fix_takes_the_value_of_the_period_it_falls_in() {
        ...
        let files = loaded_files_of(track_over(
            fixtures::utc_instant(2026, 7, 20, 0, 0) + chrono::TimeDelta::minutes(29),
            4,
            30,
        ));
    ...
    fn a_track_past_every_archived_period_has_no_plot_series() {
        ...
        archive_first_periods_of(&store, archived);
        scheduler.archived_days.insert(archived);
        let files = loaded_files_of(track_over(fixtures::utc_instant(2026, 7, 20, 12, 0), 4, 30));
    ...
    fn a_day_archived_for_one_index_values_only_that_line() {
        let (_dir, store, mut scheduler) = scheduler_with_archive();
        let archived = fixtures::date(1970, 1, 1);
    ...
    fn a_track_with_no_archived_day_has_no_plot_series() {
        let (_dir, _store, mut scheduler) = scheduler_with_archive();
        let files = loaded_files_of(track_over(fixtures::utc_instant(2026, 7, 20, 0, 0), 4, 30));
    ...
    fn the_context_lines_carry_every_archived_period() {
        ...
        let midnight = fixtures::utc_instant(2026, 7, 20, 0, 0).timestamp() as f64;
        assert_eq!(
            lines
                .hp30
                .iter()
    ...
    fn an_unarchived_day_between_two_archived_ones_breaks_the_lines() {
        let (_dir, store, mut scheduler) = scheduler_with_archive();
        for archived in [fixtures::date(2026, 7, 20), fixtures::date(2026, 7, 22)] {
            archive_first_periods_of(&store, archived);
            scheduler.archived_days.insert(archived);
        }

        let lines = context_lines_over(
            &mut scheduler,
            fixtures::date(2026, 7, 20)..=fixtures::date(2026, 7, 22),
        );
    ...
    fn revising_an_archived_day_redraws_the_context_lines() {
        ...
        archive_day(&store, archived, |archive| {
            archive.insert_or_replace_kp_day(
                ...
                Utc::now(),
                &KpSeries {
                    samples: vec![kp_sample(fixtures::utc_instant(2026, 7, 20, 0, 0), 7.667)],
                },
            )
        });
        scheduler
