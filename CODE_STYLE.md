...
| **tick level** | How strongly the line table draws one row's timestamp: strong where the hour or the day differs from the row above it, plain where the minute does, weak otherwise. A line with no fix inside its association window never draws strong. Not "emphasis" or "highlight". | `TimestampTickLevel`, `ClockTicks` |
...
| **level** | The severity an entry's message states, in one of four forms: a bracketed token (`[WARN can::session_pool]`), a bare word before a colon (`INFO:`), a bare upper-case word before whitespace (`INFO fwu_backend:`), or an angle-bracketed token (`<info>`). The last two follow an optional timestamp the service wrote itself. Not "severity" on its own, and not "log level" - that is the application's own logging. | `LogLevelKind`, `RecognisedLevel` |
