...
| **shelved** | A track taken out of a recording's working set in the history database, which keeps its nav points and comes back when the user unshelves it. The ordinary state of a stored track is `TrackState::Live`. Not "hidden" - that is per-track visibility on the map, which the settings file keeps per recording in history and the session keeps for a recording outside it. Not "archived" - those are the environment day archives. | `TrackState::Shelved`, `set_tracks_shelved`, `RecordingEntry::shelved_tracks` |
