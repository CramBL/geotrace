...
| **recording key** | What an anchor names its recording by: `Stored(DatabaseRef)` for a recording in the history database, `Session(LoadedFileId)` for one that is not. | `RecordingKey`, `unload_anchored_to` |
