...
| **boot divider** | The line table row opening a boot session, naming the run it starts. Not "reboot separator" - that is the line of the log the session was cut at. | `LineTableRow::BootDivider`, `boot_divider_row_ui` |
| **day divider** | The line table row opening a new UTC day, naming its date. Drawn above the boot divider where a boot session opens the day. Not "date separator" - a separator is a structural line of the log itself. | `DayDivider`, `LineTableRow::DayDivider` |
